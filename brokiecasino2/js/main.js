    ...
    if (!toneStarted && typeof Tone !== 'undefined') {
        try {
            ...
            noiseSynth = new Tone.NoiseSynth({
                noise: { type: 'white' }, // Or 'pink', 'brown'
                envelope: { attack: 0.01, decay: 0.1, sustain: 0 } // Short envelope
            }).toDestination();
...
function playSound(type, value = 0) {
    ...
    try {
        ...
        switch (type) {
            // General UI / Win/Loss
            case 'win_small': polySynth.triggerAttackRelease(["C4", "E4", "G4"], "8n", now); break; // Simple C major chord
            case 'win_medium': polySynth.triggerAttackRelease(["C5", "E5", "G5"], "4n", now); break; // Higher C major chord
            case 'win_big': polySynth.triggerAttackRelease(["C4", "E4", "G4", "C5", "E5", "G5"], "2n", now); break; // Fuller C major chord
            case 'lose': synth.triggerAttackRelease("C3", "4n", now); break; // Low C note
            case 'loan': synth.triggerAttackRelease("A4", "8n", now); break; // Loan/ATM sound
            case 'click': synth.triggerAttackRelease("C5", "16n", now + 0.01); break; // Quick high C for clicks

            // --- Game Specific Sounds ---
            // Slots
            case 'spin_start': noiseSynth.triggerAttackRelease("8n", now); break; // Short burst of noise
            case 'reel_stop': synth.triggerAttackRelease("A3", "16n", now + index * 0.05); break; // Staggered low A notes based on reel index
            ...
            case 'crash_tick':
                ...
                break;
            case 'crash_cashout': polySynth.triggerAttackRelease(["G4", "C5", "E5"], "4n", now); break; // Positive cashout sound
            case 'crash_explode':
            ...
            // Minefield
            case 'mine_reveal': synth.triggerAttackRelease("C5", "16n", now + 0.02); break; // Safe reveal click
            case 'mine_bomb': polySynth.triggerAttackRelease(["C3", "D#3", "A3"], "4n", now); break; // Dissonant bomb sound

            // Memory
            case 'memory_flip': synth.triggerAttackRelease("E5", "16n", now + 0.01); break; // Card flip sound
            case 'memory_match': polySynth.triggerAttackRelease(["C5", "G5"], "8n", now); break; // Simple match sound (perfect fifth)
            case 'memory_mismatch': synth.triggerAttackRelease("A3", "8n", now); break; // Mismatch sound
            case 'memory_win': polySynth.triggerAttackRelease(["C4", "E4", "G4", "C5", "E5"], "2n", now); break; // Game win sound
            case 'memory_lose': polySynth.triggerAttackRelease(["C3", "E3", "G3"], "2n", now); break; // Game lose sound

            // Horse Race
            case 'race_start': noiseSynth.triggerAttackRelease("4n", now); break; // Starting gun noise
            case 'race_win': polySynth.triggerAttackRelease(["C4", "G4", "C5", "E5"], "1n", now); break; // Fanfare win sound
            case 'race_step': noiseSynth.triggerAttackRelease("64n", now); break; // Very short noise clicks for steps

            // Roulette
            case 'roulette_spin': noiseSynth.triggerAttackRelease("1n", now); break; // Wheel spinning noise
            case 'roulette_ball': synth.triggerAttackRelease("G5", "32n", now); break; // Ball clicking sound
            case 'roulette_win': polySynth.triggerAttackRelease(["D4", "F#4", "A4", "D5"], "2n", now); break; // D major win sound

            // Blackjack
            case 'blackjack_deal': polySynth.triggerAttackRelease(["C4", "E4"], "16n", now); break; // Dealing cards sound
            case 'blackjack_hit': synth.triggerAttackRelease("D4", "16n", now + 0.01); break; // Hit sound
            case 'blackjack_bust': synth.triggerAttackRelease("A2", "4n", now); break; // Bust sound (low)
            case 'blackjack_win': polySynth.triggerAttackRelease(["C4", "G4", "C5"], "4n", now); break; // Win sound
            case 'blackjack_push': synth.triggerAttackRelease("E4", "8n", now); break; // Push sound
            case 'blackjack_blackjack': polySynth.triggerAttackRelease(["C4", "E4", "G4", "C5"], "2n", now); break; // Big Blackjack win sound

            // Plinko
            case 'plinko_drop': synth.triggerAttackRelease("E5", "16n", now); break; // Ball drop sound
            case 'plinko_peg_hit': synth.triggerAttackRelease("A5", "64n", now + Math.random() * 0.02); break; // Peg hit sound (slightly random)
            case 'plinko_win_low': polySynth.triggerAttackRelease(["C4", "E4"], "16n", now); break; // Low win sound
            case 'plinko_win_high': polySynth.triggerAttackRelease(["C5", "G5"], "8n", now); break; // High win sound
...
function addWinToLeaderboard(type, winAmount) {
    ...
    leaderboard.push({ type: type, win: winAmount }); // Add new win object
...
function setActiveTab(selectedTab) {
    ...
    if (currentActiveTab && currentActiveTab !== selectedTab) {
        ...
        if (currentActiveTab === tabCrash && typeof endCrashGame === 'function' && typeof crashGameActive !== 'undefined' && crashGameActive) { endCrashGame(true, 0, true); /* Args: crashed, multiplier, silent */ }
        ...
        if (currentActiveTab === tabRoulette && typeof resetRoulette === 'function' && typeof rouletteIsSpinning !== 'undefined' && rouletteIsSpinning) { resetRoulette(true); /* Pass true to reset wheel visually immediately */ }
        if (currentActiveTab === tabBlackjack && typeof resetBlackjack === 'function' && typeof blackjackActive !== 'undefined' && blackjackActive) { resetBlackjack(true); /* Arg: isSwitchingTabs */ }
    ...
    allTabs.forEach((tab, index) => {
        ...
        if (tab === selectedTab) {
            ...
            if (tab === tabRoulette && typeof createRouletteBettingGrid === 'function' && !document.querySelector('#roulette-inside-bets button')) { createRouletteBettingGrid(); if(typeof resetRoulette === 'function') resetRoulette(false); } // Reset without visual wheel reset
            if (tab === tabBlackjack && typeof resetBlackjack === 'function' && (typeof blackjackActive === 'undefined' || !blackjackActive)) { resetBlackjack(false); /* Arg: isSwitchingTabs = false */ }
...
function adjustBet(inputElement, amount, operation) {
    ...
    switch (operation) {
        ...
        case 'multiply': newBet = Math.floor(currentBet * amount); break; // Use floor to keep integer bets
        case 'divide': newBet = Math.floor(currentBet / amount); break; // Use floor
